...
/// Another confirmer with every permission question already answered yes.
///
/// What `--dangerously-skip-permissions` is, and the name is unpleasant on purpose. Writes, runs and
/// reading a command's output stop being put to anyone, and a quarantined file is vouched for on
/// sight, which gives up the containment this program has against a file that turns out to hold
/// instructions rather than data. It is the wrong thing everywhere the blast radius is not already
/// bounded by something else, which in practice means a container with no network and nothing in it
/// worth losing.
///
...
/// speaking unprompted. Answering either here would put words in the mouth of someone who is sitting
/// in front of the session.
///
/// Carries whether it is skipping rather than being chosen between, so a caller wires one confirmer
/// and the flag decides what it does. The alternative is two branches of an otherwise identical
/// turn, and a permission bug lives very comfortably in the branch nobody reads.
pub struct SkipsPermissions<'a, C: Confirmer> {
    inner: &'a mut C,
    skipping: bool,
}

impl<'a, C: Confirmer> SkipsPermissions<'a, C> {
    pub fn new(inner: &'a mut C, skipping: bool) -> Self {
        Self { inner, skipping }
    }
}

impl<C: Confirmer> Confirmer for SkipsPermissions<'_, C> {
    fn confirm_write(&mut self, request: &WriteRequest) -> Decision {
        match self.skipping {
            true => Decision::Approve,
            false => self.inner.confirm_write(request),
        }
    }

    /// Approves the run without vouching for its programs. Remembering would write a standing
    /// permission into the session record, where it outlives the flag: that record can be resumed
    /// without it, and the vouched list would then be claiming a person approved programs nobody
    /// was ever shown. Nothing is lost by declining to remember, since nothing is asking.
    fn confirm_run(&mut self, request: &RunRequest) -> RunDecision {
        match self.skipping {
            true => RunDecision::approve(),
            false => self.inner.confirm_run(request),
        }
    }
    ...
    fn confirm_read_output(&mut self, request: &OutputRequest) -> Decision {
        match self.skipping {
            true => Decision::Approve,
            false => self.inner.confirm_read_output(request),
        }
    }

    /// Vouches for the file, which is the part of this that gives up the most: the label on those
    /// bytes is what keeps a file's contents from being read as instructions, and this hands it over
    /// for every quarantined file the planner asks for.
    fn confirm_vouch(&mut self, request: &VouchRequest) -> Decision {
        match self.skipping {
            true => Decision::Approve,
            false => self.inner.confirm_vouch(request),
        }
    }

    /// Always the inner confirmer's. A question the planner posed is not a permission, and an
    /// answer invented here would be reported to the model as the user's own words.
    fn ask_user(&mut self, asking: &Asking) -> Vec<Answer> {
...
mod tests {
    ...
    /// The whole of what the flag buys: the four questions that ask for consent stop being asked,
    /// wrapped around the confirmer that would otherwise have refused every one of them.
    #[test]
    fn skipping_permissions_approves_every_permission_question() {
        let mut refusing = Unattended;
        let mut skipping = SkipsPermissions::new(&mut refusing, true);

        assert_eq!(skipping.confirm_write(&a_write()), Decision::Approve);
        assert!(skipping.confirm_run(&a_run()).approved());
        assert_eq!(skipping.confirm_read_output(&an_output()), Decision::Approve);
        assert_eq!(skipping.confirm_vouch(&a_vouch()), Decision::Approve);
    }

    /// A run approved by the flag must not be remembered. The vouched list goes into the session
    /// record, which can be resumed without the flag, and it would then claim a person approved
    /// programs they were never shown.
    #[test]
    fn a_skipped_run_vouches_for_nothing() {
        let mut refusing = Unattended;
        let answer = SkipsPermissions::new(&mut refusing, true).confirm_run(&a_run());
        assert!(answer.approved());
        assert!(!answer.remember, "a standing permission outlived the flag");
    }

    /// A question the planner posed asks for information, not consent, and an interjection is the
    /// person speaking unprompted. Answering either here would put words in their mouth that the
    /// planner then treats as theirs.
    #[test]
    fn skipping_permissions_still_leaves_the_questions_to_the_person() {
        let mut choosing = ChoosesFirst;
        let mut skipping = SkipsPermissions::new(&mut choosing, true);
        assert_eq!(
            skipping.ask_user(&a_series()),
            vec![Answer::Chosen(vec![0]), Answer::Declined],
            "the flag answered a question that was not a permission"
        );
    }

    /// Off, the wrapper has to be invisible: every answer is the one the wrapped confirmer gave.
    /// This is what makes wiring it unconditionally safe, so there is no second branch of the turn
    /// for a permission bug to live in.
    #[test]
    fn not_skipping_passes_every_answer_through_untouched() {
        let mut refusing = Unattended;
        let mut passing = SkipsPermissions::new(&mut refusing, false);

        assert_eq!(passing.confirm_write(&a_write()), Decision::Reject);
        assert!(!passing.confirm_run(&a_run()).approved());
        assert_eq!(passing.confirm_read_output(&an_output()), Decision::Reject);
        assert_eq!(passing.confirm_vouch(&a_vouch()), Decision::Reject);
        assert!(passing.ask_user(&a_series()).is_empty());
        assert_eq!(passing.interjection(), None);
    }
