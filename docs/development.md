...
and bravebot reads `AGENTS.md` at the workspace root and skills from `.bravebot/skills`, so
a fresh clone links the one source into all three:
...
Slash commands and subagents in this tree are Claude Code concepts, so `agents/commands/` and
`agents/agents/` link into `.claude/` alone. Neither has to exist: whatever is present is linked,
and a directory added later needs no change to the script.
